...
```python
p = Director.objects.get(name='봉준호')
...
for x in target:
    print(x.director, x.genre, x.title, x.opening_date, x.running_time, x.screening)
```
...
```python
for x in target:
    print(x.director.name, x.genre, x.title, x.opening_date, x.running_time, x.screening)
```
...
```python
x = Movie.objects.order_by('opening_date')[0]
...
```python
number = Director.objects.get(name='봉준호').id
target = Movie.objects.filter(director = number).order_by('opening_date')
...
```python
number = Director.objects.get(name='봉준호').id
x = Movie.objects.filter(director = number).order_by('opening_date')[1]
...
target = Movie.objects.filter(opening_date__range = (target_date, end_date)).order_by('opening_date__year','opening_date__month','opening_date__day')
...
```python
# 봉준호 감독 영화 중 장르가 드라마인 영화들을 개봉일 순으로 조회해서 출력하는 코드를 작성하세요.

p = Director.objects.get(name='봉준호').id
q = Genre.objects.get(title='드라마').id
target = Movie.objects.filter(director = p) & Movie.objects.filter(genre = q).order_by('opening_date__year','opening_date__month','opening_date__day')
...
target = Movie.objects.exclude(director = p).order_by('opening_date__year','opening_date__month','opening_date__day')
